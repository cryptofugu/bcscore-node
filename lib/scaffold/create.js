...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'bcscore-lib': 'https://github.com/cryptofugu/bcscore-lib.git#master',
    'bcscore-node': 'https://github.com/cryptofugu/bcscore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bcscore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bcscore-node directory and configuration
