...
/**
 * Provides a friendly event driven API to bcsd in Node.js. Manages starting and
 * stopping bcsd as a child process for application support, as well as connecting
 * to multiple bcsd processes for server infrastructure. Results are cached in an
...
util.inherits(BCS, Service);

BCS.dependencies = [];

BCS.DEFAULT_MAX_TXIDS = 1000;
BCS.DEFAULT_MAX_HISTORY = 50;
BCS.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
BCS.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
BCS.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
BCS.DEFAULT_SPAWN_RESTART_TIME = 5000;
BCS.DEFAULT_SPAWN_STOP_TIME = 10000;
BCS.DEFAULT_TRY_ALL_INTERVAL = 1000;
BCS.DEFAULT_REINDEX_INTERVAL = 10000;
BCS.DEFAULT_START_RETRY_INTERVAL = 5000;
BCS.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
BCS.DEFAULT_TRANSACTION_CONCURRENCY = 5;
BCS.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
BCS.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || BCS.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || BCS.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || BCS.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || BCS.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || BCS.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || BCS.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || BCS.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || BCS.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || BCS.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || BCS.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

BCS.prototype._initCaches = function() {
...
BCS.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
BCS.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
BCS.prototype.getPublishEvents = function() {
  return [
    {
...
BCS.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'bcsd/' + name, 'total:', this.subscriptions[name].length);
};

BCS.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bcsd/' + name, 'total:', this.subscriptions[name].length);
};


BCS.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
BCS.prototype.unsubscribeBalance = function(emitter, addresses) {
...
BCS.prototype.unsubscribeBalanceAll = function(emitter) {
...
BCS.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'bcsd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

BCS.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bcsd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
BCS.prototype.unsubscribeAddressAll = function(emitter) {
...
BCS.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = BCS.DEFAULT_CONFIG_SETTINGS;
...
BCS.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
BCS.prototype._expandRelativeDatadir = function() {
...
BCS.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in bcsd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in bcsd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in bcsd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './bcs.conf');

  log.info('Using bcs config file:', configPath);
...
BCS.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of bcscore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bcscore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bcscore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bcsd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from bcsd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from bcsd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bcsd is undergoing a reindex. ' +
             ...
             'of bcscore-node services will start.');
    node._reindex = true;
  }
};

BCS.prototype._resetCaches = function() {
...
BCS.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
BCS.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

BCS.prototype._initChain = function(callback) {
  var self = this;
...
BCS.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
BCS.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/bcs.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/bcs.conf';
    }
  }
  return networkPath;
};

BCS.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
BCS.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('bcsd/hashblock', message.toString('hex'));
    }
  }

};

BCS.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
BCS.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('BCS Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
BCS.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
BCS.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
BCS.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
BCS.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('bcsd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
BCS.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('bcsd/rawtransaction', message.toString('hex'));
    }
...
BCS.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || BCS.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

BCS.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
BCS.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
BCS.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('BCS Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || BCS.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

BCS.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
BCS.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/bcsd.pid';
...
BCS.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('BCS process unexpectedly exited with code:', code);
        log.warn('Restarting bcs child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('BCS process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn bcsd.'));
      }
...
BCS.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to bcsd.'));
    }
 ...
 */
BCS.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('BCS configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
BCS.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.syncPercentage = function(callback) {
  var self = this;
...
BCS.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, bcscore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
BCS.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
BCS.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
BCS.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
BCS.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in bcscore is most recent to less recent
 ...
 * @param {Function} callback
 */
BCS.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
BCS.prototype._getConfirmationsDetail = function(transaction) {
...
BCS.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
BCS.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
BCS.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A bcscoin transaction id
 * @param {Function} callback
 */
BCS.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
BCS.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
BCS.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
BCS.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
BCS.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
BCS.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
BCS.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
BCS.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a BCScore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
BCS.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getInfo = function(callback) {
...
BCS.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('bcsd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('bcsd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
BCS.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BCS.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
BCS.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
BCS.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
BCS.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
BCS.prototype.getStakingInfo = function(callback) {
    var self = this,
