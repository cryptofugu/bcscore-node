...
var Transaction = bcscore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/bcs.conf')));
var BCSService = proxyquire('../../lib/services/bcsd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultBCSConf = fs.readFileSync(path.resolve(__dirname, '../data/default.bcs.conf'), 'utf8');

describe('BCS Service', function() {
	...
	var baseConfig = {
		node: {
			network: bcscore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var bcsd = new BCSService(baseConfig);
			should.exist(bcsd);
		});
		it('will create an instance without `new`', function() {
			var bcsd = BCSService(baseConfig);
			should.exist(bcsd);
		});
		it('will init caches', function() {
			var bcsd = new BCSService(baseConfig);

			should.exist(bcsd.utxosCache);
			should.exist(bcsd.txidsCache);
			should.exist(bcsd.balanceCache);
			should.exist(bcsd.summaryCache);
			should.exist(bcsd.transactionDetailedCache);
			should.exist(bcsd.accountInfo);

			should.exist(bcsd.transactionCache);
			should.exist(bcsd.rawTransactionCache);
			should.exist(bcsd.rawJsonTransactionCache);
			should.exist(bcsd.blockCache);
			should.exist(bcsd.blockJsonCache);
			should.exist(bcsd.blockSubsidyCache);
			should.exist(bcsd.rawBlockCache);
			should.exist(bcsd.blockHeaderCache);
			should.exist(bcsd.zmqKnownTransactions);
			should.exist(bcsd.zmqKnownBlocks);
			should.exist(bcsd.lastTip);
			should.exist(bcsd.lastTipTimeout);
			should.equal(bcsd.dgpInfoCache, null, 'should be null');
			should.equal(bcsd.miningInfoCache, null, 'should be null');
			should.equal(bcsd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(bcsd.maxTxids, 1000);
			should.equal(bcsd.maxTransactionHistory, 50);
			should.equal(bcsd.maxAddressesQuery, 10000);
			should.equal(bcsd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(bcsd.spawnRestartTime, 5000);
			should.equal(bcsd.spawnStopTime, 10000);

			// try all interval
			should.equal(bcsd.tryAllInterval, 1000);
			should.equal(bcsd.startRetryInterval, 5000);

			// rpc limits
			should.equal(bcsd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(bcsd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.nodes.should.deep.equal([]);
			bcsd.nodesIndex.should.equal(0);
			bcsd.nodes.push({ client: sinon.stub() });
			should.exist(bcsd.client);
		});
		it('will set subscriptions', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd._initDefaults({ transactionConcurrency: 10 });
			bcsd.transactionConcurrency.should.equal(10);
			bcsd._initDefaults({});
			bcsd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			BCSService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var bcsd = new BCSService(baseConfig);
			var methods = bcsd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var bcsd = new BCSService(baseConfig);
			var events = bcsd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('bcsd/rawtransaction');
			events[0].scope.should.equal(bcsd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('bcsd/hashblock');
			events[1].scope.should.equal(bcsd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('bcsd/addresstxid');
			events[2].scope.should.equal(bcsd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('bcsd/addressbalance');
			events[3].scope.should.equal(bcsd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.subscribe = sinon.stub();
			bcsd.unsubscribe = sinon.stub();
			var events = bcsd.getPublishEvents();

			events[0].subscribe('test');
			bcsd.subscribe.args[0][0].should.equal('rawtransaction');
			bcsd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			bcsd.unsubscribe.args[0][0].should.equal('rawtransaction');
			bcsd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			bcsd.subscribe.args[1][0].should.equal('hashblock');
			bcsd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			bcsd.unsubscribe.args[1][0].should.equal('hashblock');
			bcsd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter = {};
			bcsd.subscribe('hashblock', emitter);
			bcsd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			bcsd.subscribe('rawtransaction', emitter2);
			bcsd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			bcsd.subscribe('hashblock', emitter1);
			bcsd.subscribe('hashblock', emitter2);
			bcsd.subscribe('hashblock', emitter3);
			bcsd.subscribe('hashblock', emitter4);
			bcsd.subscribe('hashblock', emitter5);
			bcsd.subscriptions.hashblock.length.should.equal(5);

			bcsd.unsubscribe('hashblock', emitter3);
			bcsd.subscriptions.hashblock.length.should.equal(4);
			bcsd.subscriptions.hashblock[0].should.equal(emitter1);
			bcsd.subscriptions.hashblock[1].should.equal(emitter2);
			bcsd.subscriptions.hashblock[2].should.equal(emitter4);
			bcsd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			bcsd.subscriptions.hashblock = [emitter1];
			bcsd.unsubscribe('hashblock', emitter3);
			bcsd.subscriptions.hashblock.length.should.equal(1);
			bcsd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			bcsd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(bcsd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter = new EventEmitter();
			bcsd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			bcsd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			bcsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeAddress(emitter1);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			bcsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(bcsd, 'unsubscribeAddressAll');
			bcsd.unsubscribeAddress(emitter1);
			bcsd.unsubscribeAddressAll.callCount.should.equal(1);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			bcsd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			bcsd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			bcsd.unsubscribeAddressAll(emitter1);
			bcsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			bcsd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(bcsd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var bcsd = new BCSService(baseConfig);
			var config = bcsd._getDefaultConfig();
			config.should.equal(defaultBCSConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a bcs.conf file', function() {
			var TestBCS = proxyquire('../../lib/services/bcsd', {
				fs: {
					readFileSync: readFileSync,
			...
			var bcsd = new TestBCS(baseConfig);
			bcsd.options.spawn.datadir = '/tmp/.bcs';

			sinon.spy(bcsd, '_expandRelativeDatadir');
			sinon.spy(bcsd, '_checkConfigIndexes');
			sinon.spy(bcsd, '_getDefaultConf')
			sinon.spy(bcsd, '_getNetworkConfigPath');
			sinon.spy(bcsd, '_parseBitcoinConf');

			var node = {};
			bcsd._loadSpawnConfiguration(node);

			bcsd._expandRelativeDatadir.callCount.should.equal(1);
			bcsd._checkConfigIndexes.callCount.should.equal(1);
			bcsd._getDefaultConf.callCount.should.equal(1);
			bcsd._getNetworkConfigPath.callCount.should.equal(1);
			bcsd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(bcsd.spawn.config);
			bcsd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: bcscore.Networks.testnet,
					configPath: '/tmp/.bcscore/bcscore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			bcsd.options.spawn.datadir = './data';
			var node = {};
			bcsd._loadSpawnConfiguration(node);
			...
			bcsd.options.spawn.datadir.should.equal('/tmp/.bcscore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestBCS = proxyquire('../../lib/services/bcsd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badbcs.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var bcsd = new TestBCS(baseConfig);
			(function() {
				bcsd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(bcscore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the bcs.conf file', function() {
			var bcsd = new BCSService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				bcsd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the bcs.conf file', function() {
			var bcsd = new BCSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the bcs.conf file', function() {
			var bcsd = new BCSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the bcs.conf file', function() {
			var bcsd = new BCSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the bcs.conf file', function() {
			var bcsd = new BCSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				bcsd.transactionDetailedCache.set(keys[i], {});
				bcsd.utxosCache.set(keys[i], {});
				bcsd.accountInfo.set(keys[i], {});
				bcsd.txidsCache.set(keys[i], {});
				bcsd.balanceCache.set(keys[i], {});
				bcsd.summaryCache.set(keys[i], {});
				bcsd.blockOverviewCache.set(keys[i], {});
				bcsd.blockJsonCache.set(keys[i], {});
			}
			bcsd._resetCaches();
			should.equal(bcsd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(bcsd.utxosCache.get(keys[0]), undefined);
			should.equal(bcsd.accountInfo.get(keys[0]), undefined);
			should.equal(bcsd.txidsCache.get(keys[0]), undefined);
			should.equal(bcsd.balanceCache.get(keys[0]), undefined);
			should.equal(bcsd.summaryCache.get(keys[0]), undefined);
			should.equal(bcsd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(bcsd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			bcsd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				bcsd.nodes[0].client.getInfo.callCount.should.equal(1);
				bcsd.nodes[1].client.getInfo.callCount.should.equal(1);
				bcsd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			bcsd._tryAllClients(function(client, next) {
				...
				bcsd.nodes[1].client.getInfo.callCount.should.equal(1);
				bcsd.nodes[2].client.getInfo.callCount.should.equal(1);
				bcsd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert bcsd-rpc error object into JavaScript error', function() {
			var bcsd = new BCSService(baseConfig);
			var error = bcsd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			bcsd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			bcsd.nodes.push({
				client: {
			...
			bcsd._initChain(function() {
				log.info.callCount.should.equal(1);
				bcsd.getRawBlock.callCount.should.equal(1);
				bcsd.getRawBlock.args[0][0].should.equal('genesishash');
				bcsd.height.should.equal(5000);
				bcsd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			bcsd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			bcsd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			bcscore.Networks.disableRegtest();
			baseConfig.node.network = bcscore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: bcscore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var bcsd = new BCSService(config);
			bcsd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var bcsd = new BCSService(config);
			bcsd._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var bcsd = new BCSService(config);
			should.equal(bcsd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var bcsd = new BCSService(config);
			bcsd._getNetworkConfigPath().should.equal('testnet3/bcs.conf');
		});
		it('will get default rpc port for regtest', function() {
			bcscore.Networks.enableRegtest();
			var config = {
			...
			var bcsd = new BCSService(config);
			bcsd._getNetworkConfigPath().should.equal('regtest/bcs.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.node.network = bcscore.Networks.testnet;
			bcsd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.node.network = bcscore.Networks.testnet;
			bcscore.Networks.enableRegtest();
			bcsd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd.node.network = bcscore.Networks.livenet;
			bcscore.Networks.enableRegtest();
			should.equal(bcsd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			bcsd._rapidProtectedUpdateTip = sinon.stub();
			bcsd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			bcsd._zmqBlockHandler(node, message);
			bcsd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			bcsd.on('block', function(block) {
				block.should.equal(message);
				bcsd.emit.callCount.should.equal(1);
				bcsd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			bcsd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			bcsd._zmqBlockHandler(node, message);
			bcsd._rapidProtectedUpdateTip.callCount.should.equal(1);
			bcsd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			bcsd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(bcsd, 'emit');
			var emitter = new EventEmitter();
			bcsd.subscriptions.hashblock.push(emitter);
			emitter.on('bcsd/hashblock', function(blockHash) {
				bcsd._rapidProtectedUpdateTip.callCount.should.equal(1);
				bcsd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				bcsd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub();
			sinon.spy(bcsd, '_resetCaches');
			bcsd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				bcsd._resetCaches.callCount.should.equal(1);
			...
			bcsd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			bcsd.on('synced', function() {
				bcsd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			bcsd.on('synced', function() {
				bcsd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			bcsd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				bcsd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub();
			bcsd._resetCaches = sinon.stub();
			bcsd.on('tip', function(height) {
				bcsd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				bcsd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using bcscore.Transaction', function() {
			var bcsd = new BCSService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = bcscore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(bcscore.Networks.testnet);
			var outputAddress = bcscore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = bcscore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: bcscore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = bcsd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(bcscore.Transaction.Output({
				script: bcscore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = bcsd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var bcsd = new BCSService(baseConfig);
			var tx = bcscore.Transaction();
			var address = bcscore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(bcscore.Transaction.Output({
				script: bcscore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(bcscore.Transaction.Output({
				script: bcscore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			bcsd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			bcsd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('bcsd/addresstxid', function(data) {
			...
			bcsd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			bcsd.subscriptions.rawtransaction.push(emitter);
			emitter.on('bcsd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			bcsd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(bcsd, 'emit');
			bcsd.subscriptions.rawtransaction.push(emitter);
			emitter.on('bcsd/rawtransaction', function() {
				done();
			});
			var node = {};
			bcsd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._updateTip = sinon.stub();
			bcsd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			bcsd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			bcsd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				bcsd._updateTip.callCount.should.equal(11);
				bcsd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				bcsd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			bcsd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				bcsd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			bcsd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				bcsd._zmqBlockHandler.callCount.should.equal(0);
				bcsd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var bcsd = new BCSService(baseConfig);
			var node = {};
			bcsd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			bcsd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			bcsd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			bcsd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestBCSService = proxyquire('../../lib/services/bcsd', {
				fs: {
					readFile: readFile
				}
			});
			var bcsd = new TestBCSService(baseConfig);
			bcsd.spawnStopTime = 1;
			bcsd._process = {};
			bcsd._process.kill = sinon.stub();
			bcsd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				bcsd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			bcsd._process.kill = sinon.stub().throws(error2);
			bcsd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._loadSpawnConfiguration = sinon.stub();
			bcsd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			bcsd._spawnChildProcess(function(err) {
				bcsd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var bcsd = new BCSService(baseConfig);
			bcsd._loadSpawnConfiguration = sinon.stub();
			bcsd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			bcsd._spawnChildProcess(function(err) {
				bcsd._loadSpawnConfiguration.callCount.should.equal(1);
				bcsd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var bcsd = new TestBCSService(config);
			bcsd.spawn = {};
			bcsd._loadSpawnConfiguration = sinon.stub();
			bcsd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			bcsd.node.stopping = true;
			bcsd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			bcsd._loadSpawnConfiguration = sinon.stub();
			bcsd.spawn = {};
			bcsd.spawn.exec = 'testexec';
			bcsd.spawn.configPath = 'testdir/bcs.conf';
			bcsd.spawn.datadir = 'testdir';
			bcsd.spawn.config = {};
			bcsd.spawn.config.rpcport = 20001;
			bcsd.spawn.config.rpcuser = 'bcs';
			bcsd.spawn.config.rpcpassword = 'password';
			bcsd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			bcsd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			bcsd._initZmqSubSocket = sinon.stub();
			bcsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bcsd._checkReindex = sinon.stub().callsArgWith(1, null);
			bcsd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/bcs.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				bcsd._loadTipFromNode.callCount.should.equal(1);
				bcsd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(bcsd._initZmqSubSocket.args[0][0].client);
				bcsd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				bcsd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(bcsd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn bcsd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestBCSService = proxyquire('../../lib/services/bcsd', {
				fs: {
			...
			bcsd._loadSpawnConfiguration = sinon.stub();
			bcsd.spawn = {};
			bcsd.spawn.exec = 'bcsd';
			bcsd.spawn.datadir = '/tmp/bcs';
			bcsd.spawn.configPath = '/tmp/bcs/bcs.conf';
			bcsd.spawn.config = {};
			bcsd.spawnRestartTime = 1;
			bcsd._loadTipFromNode = sinon.stub().callsArg(1);
			bcsd._initZmqSubSocket = sinon.stub();
			bcsd._checkReindex = sinon.stub().callsArg(1);
			bcsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bcsd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(bcsd, '_spawnChildProcess');
			bcsd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						bcsd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			bcsd._spawnChildProcess(function(err) {
				...
				bcsd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				bcsd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn bcsd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestBCSService = proxyquire('../../lib/services/bcsd', {
				fs: {
			...
			bcsd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						bcsd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			bcsd.spawn.config.rpcpassword = 'password';
			bcsd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			bcsd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			bcsd._spawnChildProcess(function(err) {
				bcsd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			bcsd._initZmqSubSocket = sinon.stub();
			bcsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bcsd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var bcsd = new BCSService(config);
			bcsd.node.stopping = true;
			bcsd.startRetryInterval = 100;
			bcsd._loadTipFromNode = sinon.stub();
			bcsd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to bcsd/);
				bcsd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			bcsd.startRetryInterval = 1;
			var config = {};
			bcsd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				bcsd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			bcsd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			bcsd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				bcsd._loadTipFromNode.callCount.should.equal(1);
				bcsd._initZmqSubSocket.callCount.should.equal(1);
				bcsd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.options = {};
			bcsd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('BCS configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			bcsd.options = {
				connect: [
					{}
				]
			...
			bcsd.start(function(err) {
				bcsd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var bcsd = new BCSService(baseConfig);
			var node = {};
			bcsd._initChain = sinon.stub().callsArg(0);
			bcsd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			bcsd.options = {
				spawn: {}
			};
			bcsd.start(function(err) {
				should.not.exist(err);
				bcsd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(bcsd, '_spawnChildProcess');
			var nodes = [{}];
			bcsd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			bcsd.options = {
				connect: [
					{}
				]
			...
			bcsd.start(function(err) {
				should.not.exist(err);
				bcsd._initChain.callCount.should.equal(1);
				bcsd._connectProcess.callCount.should.equal(1);
				bcsd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			bcsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				bcsd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			bcsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			bcsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			bcsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.syncPercentage(function(err) {
				should.exist(err);
				bcsd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			bcsd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				bcsd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var bcsd = new BCSService(baseConfig);
			var args = bcsd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var bcsd = new BCSService(baseConfig);
			var args = bcsd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_normalizeAddressArg');
			bcsd.nodes.push({
				client: {
			...
			var options = {};
			bcsd.getAddressBalance(address, options, function(err) {
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			bcsd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				bcsd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					bcsd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			bcsd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = bcsd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = bcsd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = bcsd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = bcsd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				bcsd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			bcsd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(bcsd, '_getHeightRangeQuery');
			sinon.spy(bcsd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bcsd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				bcsd._getHeightRangeQuery.callCount.should.equal(1);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bcsd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				bcsd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			bcsd.getAddressTxids(address, options, function(err, txids) {
				...
				bcsd.getAddressTxids(address, options, function(err, txids) {
					...
					bcsd._getHeightRangeQuery.callCount.should.equal(2);
					bcsd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			bcsd.getAddressTxids(address, options, function(err, txids) {
				...
				bcsd.getAddressTxids(address, options, function(err, txids) {
					...
					bcsd._getHeightRangeQuery.callCount.should.equal(4);
					bcsd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bcsd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				bcsd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					bcsd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						bcsd._getHeightRangeQuery.callCount.should.equal(3);
						bcsd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var bcsd = new BCSService(baseConfig);
			bcsd.height = 10;
			var confirmations = bcsd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var bcsd = new BCSService(baseConfig);
			var result = {};
			bcsd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var bcsd = new BCSService(baseConfig);
			var result = {};
			bcsd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var bcsd = new BCSService(baseConfig);
			var result = {};
			bcsd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var bcsd = new BCSService(baseConfig);
			var result = {};
			bcsd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(bcsd, '_getAddressDetailsForInput');
			sinon.spy(bcsd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = bcsd._getAddressDetailsForTransaction(tx, addresses);
			...
			bcsd._getAddressDetailsForInput.callCount.should.equal(1);
			bcsd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_getConfirmationsDetail');
			bcsd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			bcsd.height = 300;
			var addresses = {};
			bcsd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			bcsd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				bcsd.getDetailedTransaction.callCount.should.equal(1);
				bcsd._getAddressDetailsForTransaction.callCount.should.equal(1);
				bcsd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var bcsd = new BCSService(baseConfig);
			bcsd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			bcsd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				bcsd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from bcscore addresses', function() {
			var addresses = [
				bcscore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				bcscore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var bcsd = new BCSService(baseConfig);
			var strings = bcsd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var bcsd = new BCSService(baseConfig);
			(function() {
				bcsd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_normalizeAddressArg');
			bcsd.maxAddressesQuery = 100;
			bcsd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getAddressStrings');
			bcsd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getAddressStrings');
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			bcsd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getAddressStrings');
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			bcsd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			bcsd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd._getAddressStrings.callCount.should.equal(1);
				bcsd.getAddressTxids.callCount.should.equal(1);
				bcsd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getAddressStrings');
			sinon.spy(bcsd, 'getAddressTxids');
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			bcsd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getAddressStrings');
			sinon.spy(bcsd, '_paginateTxids');
			bcsd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(bcsd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			bcsd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd._getAddressStrings.callCount.should.equal(1);
				bcsd.getAddressTxids.callCount.should.equal(1);
				bcsd._getAddressDetailedTransaction.callCount.should.equal(2);
				bcsd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			bcsd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			bcsd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			bcsd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			bcsd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			bcsd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			bcsd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			bcsd.getAddressSummary(address, options, function(err, summary) {
				bcsd._paginateTxids.callCount.should.equal(1);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd.getAddressTxids.callCount.should.equal(1);
				bcsd.getAddressBalance.callCount.should.equal(1);
				bcsd._paginateTxids.args[0][1].should.equal(0);
				bcsd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			bcsd.getAddressSummary(address, options, function(err, summary) {
				...
				bcsd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					bcsd._paginateTxids.callCount.should.equal(0);
					bcsd._normalizeAddressArg.callCount.should.equal(2);
					bcsd.getAddressTxids.callCount.should.equal(1);
					bcsd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			bcsd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				bcsd._paginateTxids.callCount.should.equal(1);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				bcsd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			bcsd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			bcsd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(bcsd, '_tryAllClients');
			bcsd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			bcsd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			bcsd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			bcsd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			bcsd.getBlock(0, function(err) {
				bcsd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as bcscore object from height', function(done) {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			bcsd.getBlock(0, function(err, block) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(bcscore.Block);
				done();
			});
		});
		it('will getblock as bcscore object', function(done) {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			bcsd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			bcsd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			bcsd.getBlock(0, function(err, block) {
				...
				bcsd.getBlock(0, function(err, block) {
					should.not.exist(err);
					bcsd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			bcsd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			bcsd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			bcsd.getBlockHeader(10, function(err) {
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			bcsd.getBlockHeader(blockhash, function(err) {
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			bcsd.getBlockHeader(0, function(err) {
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			bcsd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			bcsd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			bcsd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				bcsd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			bcsd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			bcsd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_maybeGetBlockHash');
			bcsd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			bcsd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			bcsd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				bcsd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					bcsd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			bcsd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = bcscore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			bcsd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			bcsd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			bcsd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = bcscore.Transaction();
			(function() {
				bcsd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getRawTransaction('txid', function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			bcsd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getTransaction('txid', function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			bcsd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				bcsd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(bcscore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			bcsd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				bcsd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			bcsd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			bcsd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			bcsd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			bcsd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			bcsd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			bcsd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				bcsd.getSubsidy.callCount.should.equal(1);
				bcsd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			bcsd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.spawn = {};
			bcsd.spawn.process = new EventEmitter();
			bcsd.spawn.process.kill = sinon.stub();
			bcsd.stop(done);
			bcsd.spawn.process.kill.callCount.should.equal(1);
			bcsd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			bcsd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			bcsd.spawn.process.kill.callCount.should.equal(1);
			bcsd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			bcsd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var bcsd = new BCSService(baseConfig);
			bcsd.shutdownTimeout = 300;
			bcsd.spawn = {};
			bcsd.spawn.process = new EventEmitter();
			bcsd.spawn.process.kill = sinon.stub();
			bcsd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_normalizeAddressArg');
			sinon.spy(bcsd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			bcsd.nodes.push({
			...
			bcsd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				bcsd._getBalanceFromMempool.callCount.should.equal(1);
				bcsd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			bcsd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			bcsd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			bcsd.getJsonBlock(0, function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			bcsd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			bcsd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(2);
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			bcsd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
				bcsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter = new EventEmitter();
			bcsd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(bcsd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var bcsd = new BCSService(baseConfig);
			var emitter = new EventEmitter();
			bcsd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			bcsd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			bcsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeBalance(emitter1);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			bcsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var bcsd = new BCSService(baseConfig);
			...
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bcsd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var bcsd = new BCSService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(bcsd, 'unsubscribeBalanceAll');
			bcsd.unsubscribeBalance(emitter1);
			bcsd.unsubscribeBalanceAll.callCount.should.equal(1);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bcsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			bcsd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			bcsd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			bcsd.unsubscribeBalanceAll(emitter1);
			bcsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bcsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			bcsd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(bcsd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var bcsd = new BCSService(baseConfig);
			sinon.spy(bcsd, '_notifyBalanceSubscriber');
			bcsd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			bcsd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				bcsd._notifyBalanceSubscriber.callCount.should.equal(1);
				bcsd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			bcsd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				bcsd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			bcsd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			bcsd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getDgpInfo(function(err, result) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getMiningInfo(function(err, result) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			bcsd.getStakingInfo(function(err, result) {
				should.exist(err);
				bcsd._tryAllClients.callCount.should.equal(1);
